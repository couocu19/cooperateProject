...
class Obj {
  ...
  init() {
    ...
    this.send_comment_btn.addEventListener(
      "click",
      () => {
        this.send_comment();
      },
...
class MainerText {
  ...
  init() {
    this.imgs_warpper.addEventListener(
      "click",
      (e) => {
        this.show_imgs(e);
      },
    ...
    this.praise_num.addEventListener(
      "click",
      () => {
    ...
    this.comment_num.addEventListener(
      "click",
      () => {
    ...
    this.praise_btn_el.addEventListener(
      "click",
      () => {
  ...
  render(data) {
    ...
    this.praise_btn_el.src =
      data.isPraise == 0 ? "../../pic/no_praise.png" : "../../pic/praise.png";
  }
  ...
  adopt_photos_size() {
    if (this.imgs_warpper.childElementCount == 1) {
      this.imgs_warpper.childNodes[0].style.wihth = "99%";
      return;
    }
...
let mainertext = new MainerText(
  document
    .getElementsByClassName("user_head_pic_item")[0]
...
class Comment_Obj {
  ...
  render(comment_arr) {
    ...
    for (let i = 0, len = comment_arr.length; i < len; i++) {
      let can_delete =
        app_user_id == comment_arr[i].sendUserId ? "true" : "false";
      if (app_user_id) console.log(can_delete);
...
let comment_obj = new Comment_Obj(
  document.getElementById("comment_box_wrapper")
);

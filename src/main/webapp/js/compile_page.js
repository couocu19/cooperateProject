...
class APP {
  ...
  init() {
    this.choose_btn.addEventListener(
      "click",
      (e) => {
    ...
    // 单击查看图片，双击取消图片
    this.photos_show.addEventListener(
      "click",
      (e) => {
        // this.remove_photo(e);
  ...
  show_imgs(e) {
    ...
    let items = [];
    for (
      let i = 0, len = this.assist_var.wait_show_photo.length;
      i < len;
      i++
    ) {
      // 得到base64编码的图片的宽和高
  ...
  check() {
    if (
      ...
      this.upload_imgs.files.length == 0
    ) {
